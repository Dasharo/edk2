...
/**
  Validate and extract inner FMP capsule out of an outer one.
  ...
  )
{
  ...
  UINT16                                        InnerEmbeddedDriverCount;
  ...
  Status = ValidateFmpCapsule (InnerCapsuleHeader, &InnerEmbeddedDriverCount);
  if (EFI_ERROR (Status)) {
  ...
  if (EmbeddedDriverCount != NULL) {
    *EmbeddedDriverCount = InnerEmbeddedDriverCount;
  }
