  ...
  )
{
  ...
  if (Config.TpmVersion != QEMU_TPM_VERSION_1_2) {
    DEBUG ((DEBUG_ERROR, "[TPMPP] mPpi has an unsupported TPM type for this instance\n"));
    goto InvalidPpiAddress;
  }
  ...
  mPpi->Func[PHYSICAL_PRESENCE_NO_ACTION]                             = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_ENABLE]                                = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_DISABLE]                               = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_ACTIVATE]                              = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_DEACTIVATE]                            = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_CLEAR]                                 = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_ENABLE_ACTIVATE]                       = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_DEACTIVATE_DISABLE]                    = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_SET_OWNER_INSTALL_TRUE]                = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_SET_OWNER_INSTALL_FALSE]               = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_ENABLE_ACTIVATE_OWNER_TRUE]            = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_DEACTIVATE_DISABLE_OWNER_FALSE]        = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_CLEAR_ENABLE_ACTIVATE]                 = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_SET_NO_PPI_PROVISION_FALSE]            = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_SET_NO_PPI_PROVISION_TRUE]             = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_ENABLE_ACTIVATE_CLEAR]                 = TPM_PPI_FLAGS;
  mPpi->Func[PHYSICAL_PRESENCE_ENABLE_ACTIVATE_CLEAR_ENABLE_ACTIVATE] = TPM_PPI_FLAGS;
  ...
  )
{
  ...
  *RequestConfirmed = FALSE;
  ...
  switch (mPpi->Request) {
    ...
    default:
      if (mPpi->Request >= TCG_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
        IsRequestValid = TcgPpVendorLibHasValidRequest (mPpi->Request, Flags.PPFlags, RequestConfirmed);
        if (!IsRequestValid) {
          return FALSE;
        } else {
          break;
...
/**
  ...
  mPpi buffer is external input, so this function will validate
  ...
  )
{
  ...
  if (!HaveValidTpmRequest (Flags, &RequestConfirmed)) {
    //
  ...
  if (mPpi->Request >= TCG_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
    ...
    mPpi->Response = TcgPpVendorLibExecutePendingRequest (mPpi->Request, &NewPPFlags, &ResetRequired);
  ...
  switch (mPpi->LastRequest) {
    ...
    case PHYSICAL_PRESENCE_ENABLE_ACTIVATE_CLEAR_ENABLE_ACTIVATE:
      break;
    default:
      if (mPpi->LastRequest >= TCG_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
        if (ResetRequired) {
          break;
        } else {
          return;
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "[TPMPP] Set physical presence flag failed, Status = %r\n", Status));
      return;
    }
  }

  DEBUG ((DEBUG_INFO, "[TPMPP] PpiFlags = %x\n", PpiFlags.PPFlags));
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "[TPMPP] Error when lock variable %s, Status = %r\n", PHYSICAL_PRESENCE_FLAGS_VARIABLE, Status));
      ASSERT_EFI_ERROR (Status);
    }
  }

  //
  // Initialize physical presence.
  //
  ...
  DEBUG ((DEBUG_INFO, "[TPMPP] Flags=%x, PPRequest=%x\n", PpiFlags.PPFlags, mPpi->Request));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "[TPMPP] TcgPhysicalPresenceLibNeedUserConfirm Flags=%x, PPRequest=%x\n", PpiFlags.PPFlags, mPpi->Request));
  ...
  if (!HaveValidTpmRequest (PpiFlags, &RequestConfirmed)) {
    //
    // Invalid operation request.
    //
    return FALSE;
  ...
  )
{
  ...
  //
  // Save TPM command to PPI buffer.
  //
  Status = QemuTpmInitPPI ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "[TPMPP] no PPI\n"));
  ...
  DEBUG ((DEBUG_INFO, "[TPMPP] Submit command to PreOS: PPRequest=%x\n", PpRequest));

  mPpi->Request = PpRequest;
