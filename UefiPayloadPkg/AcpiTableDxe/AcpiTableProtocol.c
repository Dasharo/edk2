...
EFIAPI
VOID
hexdump(CONST VOID *memory, UINTN length)
{
	UINTN i, j;
	UINT8 *line;
	int all_zero = 0;
	...
	for (i = 0; i < length; i += 16) {
		...
		if ((all_zero < 2) && (all_one < 2)) {
			DEBUG ((DEBUG_ERROR, "%p:", memory + i));
			for (j = 0; j < num_bytes; j++)
				DEBUG ((DEBUG_ERROR, " %02x", line[j]));
			for (; j < 16; j++)
				DEBUG ((DEBUG_ERROR, "   "));
			DEBUG ((DEBUG_ERROR, "  "));
			for (j = 0; j < num_bytes; j++)
  ...
  )
{
  ...
  UINTN                                         Entry64Num;
  UINTN                                         Idx;
  EFI_ACPI_DESCRIPTION_HEADER                   *XsdtCopy;
  UINTN                                         TableKey;
  ...
  AcpiTableInstance->Rsdp3 = (EFI_ACPI_3_0_ROOT_SYSTEM_DESCRIPTION_POINTER *) SystemTableInfo->AcpiTableBase;
  AcpiTableInstance->Rsdt3 = (EFI_ACPI_DESCRIPTION_HEADER *) (UINTN) AcpiTableInstance->Rsdp3->RsdtAddress;
  AcpiTableInstance->Xsdt = (EFI_ACPI_DESCRIPTION_HEADER *) AcpiTableInstance->Rsdp3->XsdtAddress;

  if (AcpiTableInstance->Xsdt != NULL) {
    XsdtCopy = (EFI_ACPI_DESCRIPTION_HEADER *) AllocatePool (AcpiTableInstance->Xsdt->Length);
    if (XsdtCopy != NULL) {
      CopyMem(XsdtCopy, AcpiTableInstance->Xsdt, AcpiTableInstance->Xsdt->Length);
      Entry64 = (UINT64 *)(XsdtCopy + 1);
      Entry64Num = (XsdtCopy->Length - sizeof(EFI_ACPI_DESCRIPTION_HEADER)) >> 3;
      ZeroMem(AcpiTableInstance->Xsdt + 1, Entry64Num + sizeof(UINT64));
      for (Idx = 0; Idx < Entry64Num; Idx++) {
        TableHeader = (EFI_ACPI_DESCRIPTION_HEADER *)(UINTN)Entry64[Idx];
        if (TableHeader != NULL) {
          ...
          Status = InstallAcpiTable (&AcpiTableInstance->AcpiTableProtocol, TableHeader, TableHeader->Length, &TableKey);
          if (EFI_ERROR (Status)) {
    ...
    DEBUG((DEBUG_ERROR, "XSDT not found\n"));
    return EFI_NOT_FOUND;
  }
