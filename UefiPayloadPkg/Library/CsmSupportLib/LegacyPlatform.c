  ...
  )
{
  ...
  VOID                          *Address;
  EFI_LEGACY_PIRQ_TABLE         *PirqTable;

  Checksum = 0;

  /* Get the existing PIRQ table*/
  for (Address = (VOID *)0xf0000; Address < (VOID *)0xfffff; Address += 16) {
		PirqTable = (EFI_LEGACY_PIRQ_TABLE *) Address;
		if (PirqTable->PirqTable.Signature == EFI_LEGACY_PIRQ_TABLE_SIGNATURE) {
      LocalPirqTable = (VOID **) &PirqTable;
			break;
    }
	}
  ...
  if (LocalPirqTable != NULL) {
    ...
    //
    CopyMem(&PirqTableHead, PirqTable, PTableSize);
